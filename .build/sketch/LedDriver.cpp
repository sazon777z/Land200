#line 1 "C:\\Users\\JZX\\Documents\\Arduino\\Land_Cruiser\\LedDriver.cpp"
#include "LedDriver.h"

LedDriver::LedDriver()
    : strip(NUM_LEDS, PIN_WS2812, NEO_GRB + NEO_KHZ800), stateMutex(NULL),
      lastUpdate(0), animationStep(0), currentEffect(OFF),
      solidColor(0x00D2FF), currentBrightness(150), effectSpeed(30),
      alarmState(false), alarmCarEff(ACE_BLINK), alarmLedEff(RAINBOW),
  ...
  if (alarmState) {
    ...
    if (turnSignalMode != TS_OFF) {
      ...
      if (turnSignalMode == TS_LEFT) {
        strip.setPixelColor(10, makeSignalColor(headlightsOn ? 255 : 0,
                                                headlightsOn ? 255 : 0,
                                                headlightsOn ? 255 : 0,
                                                amberBright));
        strip.setPixelColor(9, makeSignalColor(taillightsOn ? 255 : 0, 0, 0,
                                               amberBright));
      } else if (turnSignalMode == TS_RIGHT) {
        strip.setPixelColor(11, makeSignalColor(headlightsOn ? 255 : 0,
                                                headlightsOn ? 255 : 0,
                                                headlightsOn ? 255 : 0,
                                                amberBright));
        strip.setPixelColor(8, makeSignalColor(taillightsOn ? 255 : 0, 0, 0,
                                               amberBright));
...
uint32_t LedDriver::makeSignalColor(uint8_t baseR, uint8_t baseG, uint8_t baseB,
  ...
  auto blendChannel = [signalBrightness](uint8_t base, uint8_t signal) -> uint8_t {
