...
LedDriver::LedDriver() : 
    strip(NUM_LEDS, PIN_WS2812, NEO_GRB + NEO_KHZ800),
    lastUpdate(0),
    animationStep(0),
...
void LedDriver::update() {
    if (millis() - lastUpdate < (uint32_t)effectSpeed) return;
    lastUpdate = millis();
    
    // --- 1. Underglow (LEDs 0-3) ---
    if (!alarmState) {
        switch (currentEffect) {
            case OFF: for(int i=0; i<4; i++) strip.setPixelColor(i, 0); break;
            case RAINBOW: rainbowEffect(); break;
            case SOLID: for(int i=0; i<4; i++) strip.setPixelColor(i, solidColor); break;
            case BREATH: breathEffect(); break;
            case POLICE: policeEffect(); break;
            case STROME: strobeEffect(); break;
            case FADE: fadeEffect(); break;
            case SPARKLE: sparkleEffect(); break;
            case KNIGHT_RIDER: knightRiderEffect(); break;
            default: break;
        }
    }
    ...
    if (alarmState) {
        ...
        strip.setPixelColor(4, frontColor); strip.setPixelColor(5, frontColor);
        strip.setPixelColor(6, rearColor); strip.setPixelColor(7, rearColor);
        ...
        if (turnFlash && turnSignalMode != TS_OFF) {
            ...
            if (turnSignalMode == TS_LEFT) {
                strip.setPixelColor(4, amber); strip.setPixelColor(6, amber);
            } else if (turnSignalMode == TS_RIGHT) {
                strip.setPixelColor(5, amber); strip.setPixelColor(7, amber);
            } else if (turnSignalMode == TS_HAZARD) {
                strip.setPixelColor(4, amber); strip.setPixelColor(5, amber);
                strip.setPixelColor(6, amber); strip.setPixelColor(7, amber);
            }
        }
    }
    strip.show();
...
void LedDriver::breathEffect() {
    ...
    for(int i=0; i<4; i++) strip.setPixelColor(i, strip.Color(r, g, b));
    animationStep++;
}
...
void LedDriver::fadeEffect() {
    ...
    val += direction * 5;
    if (val >= 255) { val = 255; direction = -1; }
...
void LedDriver::alarmEffect() {
    ...
    if(flashCount > 10) {
        flash = !flash;
        flashCount = 0;
        // Also toggle digital headlights/taillights if any
        setHeadlights(flash);
        setTaillights(!flash);
    }
    
    // Addressable Hazard: All LEDs blink amber
    uint32_t amber = flash ? strip.Color(255, 80, 0) : 0;
    strip.fill(amber);
}
