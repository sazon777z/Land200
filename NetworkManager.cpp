...
void WatchNetworkManager::begin() {
    ...
    int retries = 0;
    while (WiFi.status() != WL_CONNECTED && retries < 40) { // Wait up to 20 seconds
        delay(500);
        Serial.print(".");
        retries++;
    }
...
void WatchNetworkManager::saveAlarm(int hour, int minute, int soundId, bool enabled) {
    alarmHour = hour;
    alarmMinute = minute;
...
bool WatchNetworkManager::checkAlarmTrigger() {
    if (!alarmEnabled || apMode || WiFi.status() != WL_CONNECTED) return false;
    ...
    if (currentH == alarmHour && currentM == alarmMinute) {
        ...
        if (lastTriggerMinute != -1) lastTriggerMinute = -1;
    }
...
void WatchNetworkManager::resetAlarmTrigger() {
    ...
    // For now, we just let the logic in checkAlarmTrigger handle the per-minute block
}
